...
### Libraries
The following are the libraries that were used in this excercise.
```{r,}
library(tidyverse)
library(stringr)
library(ggplot2)
...
### Import Data
We imported the following data from [Kaggle](https://www.kaggle.com/datasets/fabriziocominetti/the-office-lines).
```{r}
...
allline <- office |>
  mutate(line_ci = str_to_lower(Line), 
         j_c = str_detect(line_ci, "\\bjesus\\b\\s+\\bchrist\\b"))
allline |> 
  summarize(sum(j_c)) |> 
  pull()
...
```{r}
plot3 <- plot2 |> 
  mutate(office = str_count(Line2, regex("\\boffice\\b"))) |> 
  group_by(Character) |> 
  summarize(word = n(), office = sum(office)) |>
  filter(office > 0)
head(plot3)
```
